...
import { Search, User, FileText, AlertTriangle, Phone } from 'lucide-react';
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        {initialCustomers.length === 0 ? (
          ...
          <>
            ...
            <div className="block sm:hidden border-t border-slate-200 dark:border-slate-800 divide-y divide-slate-100 dark:divide-slate-800">
              {initialCustomers.map(c => (
                <div key={c.id} className="p-4 space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <div className="font-bold text-slate-900 dark:text-slate-100">{c.name}</div>
                      <div className="text-xs text-slate-500 mt-0.5">{c.customerCode} • {c.phone}</div>
                    </div>
                    {c.assignedSales && (
                  ...
                  <div className="flex justify-between items-end bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-100 dark:border-slate-700">
                    <div>
                      <p className="text-[10px] text-slate-500 uppercase font-bold">Tổng nợ ({c.orders.length} đơn)</p>
                      <p className="text-lg font-black text-rose-600">{formatCurrencyVND(c.debtBalance)}</p>
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <a href={`tel:${c.phone}`} className="bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 py-2.5 rounded-lg text-xs font-bold text-center flex items-center justify-center gap-1.5 active:bg-slate-200">
                      <Phone className="h-3.5 w-3.5" />
                      Gọi khách
                    </a>
                    <Link href={`/dashboard/print/customers/${c.id}/debt-statement`} className="bg-indigo-600 text-white py-2.5 rounded-lg text-xs font-bold text-center flex items-center justify-center gap-1.5 active:bg-indigo-700 shadow-sm">
